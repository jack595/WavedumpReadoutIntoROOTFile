...
# @File: GatherDataTogetherBetweenDigitizers.py
...
def LoadDigitizerData(path:str, v_Digitizers):
    ...
    for Digitizer in v_Digitizers:
        print(f"Loading Data of {path}/{Digitizer}.....")
        if os.path.isfile(f"{path}/{Digitizer}_1.pkl"):
            ...
            exit(1)
        df_TQ_BeforeCut.drop(['boardID', 'pattern', 'eventCounter'], axis=1, inplace=True)
        ConvertTQDataframeType(df_TQ_BeforeCut)
        ...
        ConvertTQDataframeType(df_TQ_AfterCut)
...
def MergeDatasetBetweenDigitizers(dict_df_TQ_raw:dict,
    ...
    for Digitizer, index_align_digitizer in dict_align_index_Digitizers.items():
        ...
        if len(df_TQ_MergeDigitizers):
            ...
            df_TQ_MergeDigitizers = df_TQ_tmp
    return df_TQ_MergeDigitizers

def ConvertTQDataframeType(df:pd.DataFrame):
    from tqdm import tqdm
    dict_key2Type = {"charge":np.float32, "triggerTime":np.float64, "T_":np.float16, "Q_":np.float32,
                     "amp":np.float16, "valley":np.float16,
                     "Valley":np.float16, "width":np.int,"Width":np.int32 }
    for column in tqdm(df.columns):
        for key in dict_key2Type.keys():
            if key in column:
                if isinstance(df[column].iloc[0], list) or isinstance(df[column].iloc[0], np.ndarray):
                    df[column] = df[column].apply(lambda x: np.array(x, dtype=dict_key2Type[key]).tolist())
                else:
                    df[column] = df[column].apply(lambda x: dict_key2Type[key](x) if not pd.isnull(x) else np.nan)
    return df
...
if __name__ == "__main__":
    ...
    if args.SplitSaving:
        print("Saving Concatenated Dataframe In Splitting Mode.............")
        from PandasTools import dfToPickleByChunk
        dfToPickleByChunk(df_TQ_MergeDigitizers, path_save_output)
    else:
